...
class Post
{
			...
			if($this->error == ""){
				...
				if(isset($data['parent']) && is_numeric($data['parent'])){
					...
					if(is_array($mypost) && $mypost['userid'] != $userid){
						...
						//add notification
						add_notification($_SESSION['linkup_userid'],"comment",$mypost);
					}
	...
	public function i_own_post($postid,$linkup_userid)
	{
		...
		$result = $DB->read($query);
  		...
  		if(is_array($result)){
  			...
  			if($result[0]['userid'] == $linkup_userid){
	...
	public function like_post($id,$type,$linkup_userid){
			...
			if(is_array($result)){
				...
				if(!in_array($linkup_userid, $user_ids)){

					$arr["userid"] = $linkup_userid;
					...
					if($type != "user"){
						...
						//add notification
						add_notification($_SESSION['linkup_userid'],"like",$single_post);
					}

				}else{

					$key = array_search($linkup_userid, $user_ids);
					unset($likes[$key]);
